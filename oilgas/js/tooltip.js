...
export function showTooltip(event, geoid) {
  ...
  const startField = `${res.fieldPrefix}${_state.startYear}`;
  const endField = `${res.fieldPrefix}${_state.endYear}`;
  const startVal = row[startField] || 0;
  const endVal = row[endField] || 0;
  ...
  const lines = [
    ...
    '\u2500'.repeat(36),
    res.label,
    `  ${_state.startYear}: ${fmt(startVal).padStart(8)} ${res.unit.padEnd(6)} ${barStr(startVal)}`,
    `  ${_state.endYear}: ${fmt(endVal).padStart(8)} ${res.unit.padEnd(6)} ${barStr(endVal)}`,
  ];
