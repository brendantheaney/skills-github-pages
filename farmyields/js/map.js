...
function updateLegend(state, data) {
  ...
  const metric = data.metrics.find(m => m.id === state.metric);
  if (!crop || !metric) return;

  const key = `${crop.id}_${state.metric}`;
  const maxVal = data.maxValues.get(key);
  const fmt = d3.format('.3~s');
  const unit = crop.units[state.metric];

  legendEl.textContent = [
    `  \u2591  = START YEAR (${state.startYear})    [lighter]`,
    `  \u2588  = END YEAR   (${state.endYear})    [solid]`,
    ``,
    `  Max bar = ${fmt(maxVal)} ${unit}  (${crop.label} ${metric.label})`,
